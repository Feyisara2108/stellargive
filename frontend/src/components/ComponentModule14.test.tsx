...
import { ComponentModule14 } from "./ComponentModule14";
...
describe("ComponentModule14", () => {
  beforeEach(() => {
    queryClient.clear();
  });

  it("renders correctly when not connected", async () => {
    renderWithProviders(<ComponentModule14 />, { isConnected: false, address: null });
    expect(screen.getByText("Module 14")).toBeInTheDocument();
    expect(screen.getByText("Not connected")).toBeInTheDocument();
    await waitFor(() => expect(screen.getByText("Hello from query 14")).toBeInTheDocument());
  });
  ...
  it("renders correctly when connected", async () => {
    renderWithProviders(<ComponentModule14 />, { isConnected: true, address: "G123" });
  ...
  it("handles focus, blur, and input interactions", async () => {
    renderWithProviders(<ComponentModule14 />, { isConnected: true, address: "G123" });
    const input = screen.getByTestId("mod14-input");
  ...
  it("handles button click interaction", async () => {
    renderWithProviders(<ComponentModule14 />, { isConnected: true, address: "G123" });
    const button = screen.getByTestId("mod14-button");
