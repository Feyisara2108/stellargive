...
import { ComponentModule26 } from "./ComponentModule26";
...
describe("ComponentModule26", () => {
  beforeEach(() => {
    queryClient.clear();
  });

  it("renders correctly when not connected", async () => {
    renderWithProviders(<ComponentModule26 />, { isConnected: false, address: null });
    expect(screen.getByText("Module 26")).toBeInTheDocument();
    expect(screen.getByText("Not connected")).toBeInTheDocument();
    await waitFor(() => expect(screen.getByText("Hello from query 26")).toBeInTheDocument());
  });
  ...
  it("renders correctly when connected", async () => {
    renderWithProviders(<ComponentModule26 />, { isConnected: true, address: "G123" });
  ...
  it("handles focus, blur, and input interactions", async () => {
    renderWithProviders(<ComponentModule26 />, { isConnected: true, address: "G123" });
    const input = screen.getByTestId("mod26-input");
  ...
  it("handles button click interaction", async () => {
    renderWithProviders(<ComponentModule26 />, { isConnected: true, address: "G123" });
    const button = screen.getByTestId("mod26-button");
