...
import { UIModule58 } from "./UIModule58";
...
describe("UIModule58", () => {
  it("renders correctly when not connected", async () => {
    const { container } = renderWithProviders(<UIModule58 />, {
      isConnected: false,
      address: null,
    });
    expect(screen.getByText("Module 58")).toBeInTheDocument();
    expect(screen.getByText("Not connected")).toBeInTheDocument();
    await waitFor(() => expect(screen.getByText("Hello from query 58")).toBeInTheDocument());
  ...
  it("renders correctly when connected", async () => {
    renderWithProviders(<UIModule58 />, { isConnected: true, address: "G123456789" });
  ...
  it("handles input focus, change, and blur interactions", async () => {
    renderWithProviders(<UIModule58 />, { isConnected: true, address: "G123456789" });
    const input = screen.getByTestId("mod58-input");
  ...
  it("handles button click interaction", async () => {
    renderWithProviders(<UIModule58 />, { isConnected: true, address: "G123456789" });
    const button = screen.getByTestId("mod58-button");
  ...
  it("renders loading state with status role and screen reader label", () => {
    ...
    renderWithProviders(<UIModule58 />, { isConnected: false, address: null }, client);
  ...
  it("renders error state and handles retry action", async () => {
    ...
    const mockFetch = async () => {
      ...
      return { message: "Hello from query 58" };
    };

    renderWithProviders(
      <UIModule58 fetchData={mockFetch} />,
    ...
    const retryButton = screen.getByTestId("mod58-retry");
    fireEvent.click(retryButton);

    await waitFor(() => {
      expect(screen.getByText("Hello from query 58")).toBeInTheDocument();
    });
  });
});
