...
import { ComponentModule15 } from "./ComponentModule15";
...
describe("ComponentModule15", () => {
  beforeEach(() => {
    queryClient.clear();
  });

  it("renders correctly when not connected", async () => {
    renderWithProviders(<ComponentModule15 />, { isConnected: false, address: null });
    expect(screen.getByText("Module 15")).toBeInTheDocument();
    expect(screen.getByText("Not connected")).toBeInTheDocument();
    await waitFor(() => expect(screen.getByText("Hello from query 15")).toBeInTheDocument());
  });
  ...
  it("renders correctly when connected", async () => {
    renderWithProviders(<ComponentModule15 />, { isConnected: true, address: "G123" });
  ...
  it("handles focus, blur, and input interactions", async () => {
    renderWithProviders(<ComponentModule15 />, { isConnected: true, address: "G123" });
    const input = screen.getByTestId("mod15-input");
  ...
  it("handles button click interaction", async () => {
    renderWithProviders(<ComponentModule15 />, { isConnected: true, address: "G123" });
    const button = screen.getByTestId("mod15-button");
