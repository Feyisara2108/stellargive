...
import { ComponentModule30 } from "./ComponentModule30";
...
describe('ComponentModule30', () => {
  beforeEach(() => {
    queryClient.clear();
  });

  it('renders correctly when not connected', async () => {
    renderWithProviders(<ComponentModule30 />, { isConnected: false, address: null });
    expect(screen.getByText('Module 30')).toBeInTheDocument();
    expect(screen.getByText('Not connected')).toBeInTheDocument();
    await waitFor(() => expect(screen.getByText('Hello from query 30')).toBeInTheDocument());
  });

  it('renders correctly when connected', async () => {
    renderWithProviders(<ComponentModule30 />, { isConnected: true, address: 'G123' });
    expect(screen.getByText('Connected as G123')).toBeInTheDocument();
  });

  it('handles focus and input interactions', async () => {
    renderWithProviders(<ComponentModule30 />, { isConnected: true, address: 'G123' });
    const input = screen.getByTestId('mod30-input');
  ...
  it('handles button click interaction', async () => {
    renderWithProviders(<ComponentModule30 />, { isConnected: true, address: 'G123' });
    const button = screen.getByTestId('mod30-button');
