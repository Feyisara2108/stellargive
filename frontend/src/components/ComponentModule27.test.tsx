...
import { ComponentModule27 } from "./ComponentModule27";
...
describe("ComponentModule27", () => {
  beforeEach(() => {
    queryClient.clear();
  });

  it("renders correctly when not connected", async () => {
    renderWithProviders(<ComponentModule27 />, { isConnected: false, address: null });
    expect(screen.getByText("Module 27")).toBeInTheDocument();
    expect(screen.getByText("Not connected")).toBeInTheDocument();
    await waitFor(() => expect(screen.getByText("Hello from query 27")).toBeInTheDocument());
  });
  ...
  it("renders correctly when connected", async () => {
    renderWithProviders(<ComponentModule27 />, { isConnected: true, address: "G123" });
  ...
  it("handles focus, blur, and input interactions", async () => {
    renderWithProviders(<ComponentModule27 />, { isConnected: true, address: "G123" });
    const input = screen.getByTestId("mod27-input");
  ...
  it("handles button click interaction", async () => {
    renderWithProviders(<ComponentModule27 />, { isConnected: true, address: "G123" });
    const button = screen.getByTestId("mod27-button");
