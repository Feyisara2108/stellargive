...
import { ComponentModule25 } from "./ComponentModule25";
...
describe("ComponentModule25", () => {
  beforeEach(() => {
    queryClient.clear();
  });

  it("renders correctly when not connected", async () => {
    renderWithProviders(<ComponentModule25 />, { isConnected: false, address: null });
    expect(screen.getByText("Module 25")).toBeInTheDocument();
    expect(screen.getByText("Not connected")).toBeInTheDocument();
    await waitFor(() => expect(screen.getByText("Hello from query 25")).toBeInTheDocument());
  });
  ...
  it("renders correctly when connected", async () => {
    renderWithProviders(<ComponentModule25 />, { isConnected: true, address: "G123" });
  ...
  it("handles focus, blur, and input interactions", async () => {
    renderWithProviders(<ComponentModule25 />, { isConnected: true, address: "G123" });
    const input = screen.getByTestId("mod25-input");
  ...
  it("handles button click interaction", async () => {
    renderWithProviders(<ComponentModule25 />, { isConnected: true, address: "G123" });
    const button = screen.getByTestId("mod25-button");
