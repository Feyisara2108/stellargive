...
export const UIModule60 = ({ fetchData }: UIModuleProps = {}) => {
  ...
  return (
    <div className="flex flex-col gap-4 p-4 sm:p-6 w-full max-w-lg">
      <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Module 60</h1>
      {isConnected ? (
      ...
      <input
        data-testid="mod60-input"
        value={inputValue}
        onChange={(e) => setInputValue(e.target.value)}
        onFocus={() => setIsFocused(true)}
        onBlur={() => setIsFocused(false)}
        aria-label="Module 60 input"
