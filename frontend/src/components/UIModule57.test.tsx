...
import { UIModule57 } from "./UIModule57";
...
describe("UIModule57", () => {
  it("renders correctly when not connected", async () => {
    const { container } = renderWithProviders(<UIModule57 />, {
      isConnected: false,
      address: null,
    });
    expect(screen.getByText("Module 57")).toBeInTheDocument();
    expect(screen.getByText("Not connected")).toBeInTheDocument();
    await waitFor(() => expect(screen.getByText("Hello from query 57")).toBeInTheDocument());
  ...
  it("renders correctly when connected", async () => {
    renderWithProviders(<UIModule57 />, { isConnected: true, address: "G123456789" });
  ...
  it("handles input focus, change, and blur interactions", async () => {
    renderWithProviders(<UIModule57 />, { isConnected: true, address: "G123456789" });
    const input = screen.getByTestId("mod57-input");
  ...
  it("handles button click interaction", async () => {
    renderWithProviders(<UIModule57 />, { isConnected: true, address: "G123456789" });
    const button = screen.getByTestId("mod57-button");
  ...
  it("renders loading state with status role and screen reader label", () => {
    ...
    renderWithProviders(<UIModule57 />, { isConnected: false, address: null }, client);
  ...
  it("renders error state and handles retry action", async () => {
    ...
    const mockFetch = async () => {
      ...
      return { message: "Hello from query 57" };
    };

    renderWithProviders(
      <UIModule57 fetchData={mockFetch} />,
    ...
    const retryButton = screen.getByTestId("mod57-retry");
    fireEvent.click(retryButton);

    await waitFor(() => {
      expect(screen.getByText("Hello from query 57")).toBeInTheDocument();
    });
  });
});
