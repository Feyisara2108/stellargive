...
import { ComponentModule28 } from "./ComponentModule28";
...
describe("ComponentModule28", () => {
  beforeEach(() => {
    queryClient.clear();
  });

  it("renders correctly when not connected", async () => {
    renderWithProviders(<ComponentModule28 />, { isConnected: false, address: null });
    expect(screen.getByText("Module 28")).toBeInTheDocument();
    expect(screen.getByText("Not connected")).toBeInTheDocument();
    await waitFor(() => expect(screen.getByText("Hello from query 28")).toBeInTheDocument());
  });
  ...
  it("renders correctly when connected", async () => {
    renderWithProviders(<ComponentModule28 />, { isConnected: true, address: "G123" });
  ...
  it("handles focus, blur, and input interactions", async () => {
    renderWithProviders(<ComponentModule28 />, { isConnected: true, address: "G123" });
    const input = screen.getByTestId("mod28-input");
  ...
  it("handles button click interaction", async () => {
    renderWithProviders(<ComponentModule28 />, { isConnected: true, address: "G123" });
    const button = screen.getByTestId("mod28-button");
