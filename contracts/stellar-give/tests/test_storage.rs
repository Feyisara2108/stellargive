...
fn get_campaign_ttl(env: &Env, client: &StellarGiveContractClient<'static>, campaign_id: u64) -> u32 {
...
fn test_persistent_storage_ttl_extension() {
    ...
    assert!(ttl >= EXPECTED_TTL - 10, "campaign TTL should be set at creation");
}
...
fn test_persistent_storage_ttl_extended_by_read() {
    ...
    let campaign_id =
        create_default_campaign(&env, &client, &creator, &beneficiary, &token_client.address, 2_000);
    let ttl_after_create = get_campaign_ttl(&env, &client, campaign_id);
...
fn test_persistent_storage_ttl_extended_by_write() {
    ...
    let campaign_id =
        create_default_campaign(&env, &client, &creator, &beneficiary, &token_client.address, 10_000);
    let ttl_after_create = get_campaign_ttl(&env, &client, campaign_id);
    ...
    // Writing to the campaign (donate) must extend TTL
...
fn test_transient_comment_data_is_not_persisted() {
    ...
    client.donate(&donor, &campaign_id, &1_000_000, &false, &Some(comment.clone()));
    ...
    let campaign_in_persistent =
        env.as_contract(&client.address, || env.storage().persistent().has(&campaign_key));
    assert!(campaign_in_persistent, "campaign must still be in persistent storage");
}
...
fn test_instance_vs_persistent_storage_separation() {
    ...
    let next_in_persistent =
        env.as_contract(&client.address, || env.storage().persistent().has(&next_key));
    assert!(next_in_instance, "NEXT_ID must be stored in instance storage");
    assert!(!next_in_persistent, "NEXT_ID must NOT be in persistent storage");

    // PAUSED key is stored in instance storage
    let paused_key = symbol_short!("PAUSED");
    let paused_in_instance =
        env.as_contract(&client.address, || env.storage().instance().has(&paused_key));
    assert!(paused_in_instance, "PAUSED must be stored in instance storage");
    ...
    assert!(owner_in_instance, "OWNER must be stored in instance storage");

    // Campaign keys are stored in persistent storage, not instance
    let campaign_key = (symbol_short!("CMP"), 1_u64);
    let cmp_in_persistent =
        env.as_contract(&client.address, || env.storage().persistent().has(&campaign_key));
    let cmp_in_instance =
        env.as_contract(&client.address, || env.storage().instance().has(&campaign_key));
    assert!(cmp_in_persistent, "campaign data must be in persistent storage");
    assert!(!cmp_in_instance, "campaign data must NOT be in instance storage");
}
...
fn test_donor_contribution_ttl_extended_by_read() {
    ...
    let ttl_after_write =
        env.as_contract(&client.address, || env.storage().persistent().get_ttl(&dc_key));
